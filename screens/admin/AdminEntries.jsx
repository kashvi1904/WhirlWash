    ...
    try {
      ...
      await firestore()
        ...
        .update({
          ...
          penaltyUntil: value ? new Date(Date.now() + 24*60*60*1000).toISOString() : null, // 24 hours if turning on
        });
      ...
      const updatedStudents = students.map(s => {
        if (s.id === student.id) {
          return { 
            ...s, 
            hasPenaltyNotification: value,
            penaltyUntil: value ? new Date(Date.now() + 24*60*60*1000).toISOString() : null,
          };
        }
        return s;
      });
